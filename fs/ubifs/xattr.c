...
static int setxattr(struct inode *host, const char *name, const void *value,
		    size_t size, int flags, int check_lock)
{
	struct inode *inode;
		   ...
		   const void *value, size_t size, int flags)
{
	...
	return setxattr(d_inode(dentry), name, value, size, flags, 1);
}
		      ...
		      void *fs_info)
{
	...
	/* This is a new node, the mutex lock of parent node is hold.
	   It's no need to lock the new node, and do not check the lock in setxattr(). */
	int check_lock = 0;
	...
	for (xattr = xattr_array; xattr->name != NULL; xattr++) {
		...
		err = setxattr(inode, name, xattr->value, xattr->value_len, 0, check_lock);
		kfree(name);
		if (err < 0)
			break;
	}
